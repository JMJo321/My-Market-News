...
# # 2.1. Modify the data
resp_all.reports_modified <-
  str_replace(resp_all.reports, '^\\"', "") %>%
    str_replace(., '\\"$', "") %>%
...
dt_all.reports <-
  lapply(
    resp_all.reports_modified[2:length(resp_all.reports_modified)],
...
# ------------------------------------------------------------------------------
# ------- (...) -------
# TODO: Compare the TBL and DT
# paste(api_endpoint_base, "3210", sep = "/") %>% create_req(.) %>% get_resp(.) %>% resp_body_string(.)
# paste(api_endpoint_base, "3615", sep = "/") %>% create_req(.) %>% get_resp(.) %>% resp_body_string(.)
# paste(api_endpoint_base, "3615", sep = "/") %>% create_req(.) %>% get_resp(.) %>% resp_body_json(.)
# paste(api_endpoint_base, "3615", sep = "/") %>% create_req(.) %>% get_resp(.) %>% resp_body_json(.) %>% jsonlite::toJSON(., pretty = TRUE)
